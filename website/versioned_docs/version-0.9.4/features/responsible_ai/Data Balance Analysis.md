...
| - | - | - | - | -
Demographic Parity | Fairness | Proportion of each segment of a protected class (gender, for example) should receive the positive outcome at equal rates. | Closer to 0 means better parity. \\(DP = P(Y \vert A = "Male") - P(Y \vert A = "Female")\\). Y = Positive label rate. | [Link](https://en.wikipedia.org/wiki/Fairness_%28machine_learning%29)

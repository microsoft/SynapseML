...
`synapseml-1.0.15.zip` archive is not published. Install `synapseml-core` and
the modules needed by your application (the following installs all six):

```R
devtools::install_url("https://mmlspark.blob.core.windows.net/rrr/synapseml-core-1.0.15.zip")
...
config <- spark_config()
config$sparklyr.defaultPackages <- "com.microsoft.azure:synapseml_2.12:1.0.15"
...
- add the Maven library `com.microsoft.azure:synapseml_2.12:1.0.15` and set its

...
want to run.  There is another component implicit here which is the _tsag_ (=
version) that you want to use — specifying it explicitly looks like
`mcr.microsoft.com/mmlspark/release:0.10.0` for the `0.10.0` tag.
...
docker run -it --rm \
           ...
           mcr.microsoft.com/mmlspark/release:0.10.0
```

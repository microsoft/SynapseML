...
- [x] The sibling `spark40-url-red.log:37-44` and `spark40-url-green.log:331-375` independently show the same red/green regressions, 29/29 tests across four suites and packaging success. Its ASCII SBT result records were extracted despite NUL bytes in the log. The fixed round-1 defect is not reopened.
- [x] `E\spark41-openai-stub-red.log` fails on missing public `OpenAIPrompt.pyi`; `-green.log` passes the exact changed file-only method on Python 3.13.13 / PySpark 4.1.1 and records Black passing. The sibling logs pass on Python 3.12.3 / PySpark 4.0.1. These interpreter versions are evidence, not a claim that every environment pin was reproduced.

...
- PR: #2720, targeting `spark4.1`; HEAD: `1c5f8921a7e6ebfd29dc49b24646877f1b3cc605`.
...
| P | `core\src\test\scala\com\microsoft\azure\synapse\ml\codegen\PyCodegenSuite.scala` | `5fe89b55c0dfad07c89eeeea337b9f08fb362ba66d3386f1c4983864fe92aab0` |
...
- [x] `project\CodegenPlugin.scala:266-302` derives production names from Compile artifacts and test names from Test artifacts. `PyCodegen.scala:112-118`, `RCodegen.scala:19-25`, `PyTestGen.scala:22-23` and `RTestGen.scala:21-23` pass those scopes to discovery.
...
- [x] `spark41-red-repro.log:121` records the exact fixture `NoSuchMethodException`; `spark41-green-packaging.log:70-90` records 27/27 tests across four suites, no skips, and clean core style. The same log records main/test packaging and packaged-classpath Python codegen. `spark41-r-wheel.log:288` and the packaging report establish the corrected R/wheel run; the earlier `rCodeGen` typo is resolved.
- [x] `E\spark41-openai-stub-red.log` fails on missing public `OpenAIPrompt.pyi`; `-green.log` passes the exact file-only method on Python 3.13.13 / PySpark 4.1.1 and records Black passing. The runner omitted only module-level Spark startup.
...
Impact: valid workspace names can silently remove production wrappers, since `PyCodegen.generatePythonClasses` accepts an empty discovery list and `pyGen` cleans existing output first. An artifact-named `!` directory can instead admit test fixtures into production discovery. Ordinary CI paths still pass.
...
Two compiled Scala regressions failed before the fix in `spark41-url-red.log`.
...
`spark41-url-green.log` records all-module main/test compilation and Scala

...
Worktree `sync-spark41-20260916`, HEAD `1c5f8921a7e6ebfd29dc49b24646877f1b3cc605`.
Source unchanged since the round 1 URL fix. Four files in scope:

| File | SHA256 prefix | Parity with spark4.0 |
| --- | --- | --- |
...
reads "generated runtime wrappers and stubs tolerate foreign-owned parameters";
spark4.0 reads "generated wrappers and stubs tolerate Spark 4 rejecting foreign
parameter defaults". That line is outside the repair hunk. The repair itself is the
same one-line comment change on both ports (diffstat: 2 lines touched each).
...
`spark41-final-packaging-complete.log` and
`spark41-final-packaging-evidence.json`. The generated tests were not executed
by this check; fresh remote CI remains required.
- File counts and the Wrappable mixin count came from the spark4.0 worktree; the
  three shared files are SHA256-identical here, so the analysis carries over.

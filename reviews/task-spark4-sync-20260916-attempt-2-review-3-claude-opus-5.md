# Spark 4.0 CI repair review: attempt 2, round 3
...
- PR #2718 -> `spark4.0`; worktree `sync-spark40-20260916`, HEAD `e2ea243ce83fa4a8d529d6ede7bcaa04529dfb24`.
...
| P | `core\src\test\scala\com\microsoft\azure\synapse\ml\codegen\PyCodegenSuite.scala` | `2c73e4f97aac6064aad0db7921a6fdc23b2ac9746af1ee79ff3bfb42d63631d3` | 614 |
...
- Classifier/snapshot identity holds: `-SNAPSHOT-tests` and `-tests-SNAPSHOT` both normalize to `-tests`, main vs tests never cross-match, and `-sources` is rejected on the archive path. Build-stamped snapshots cannot alias a release, because `...-1144-SNAPSHOT` normalizes to `...-1144`, not to `1.1.3`.
...
`pyTestGen` / `rTestGen` consume the same changed matcher with the test-artifact name (`core\src\test\scala\...\PyTestGen.scala`, `RTestGen.scala`, wired at `CodegenPlugin.scala:168-171,227-230`), but neither ran locally: `spark40-url-green.log` and `spark41-url-green.log` contain zero `testgen` mentions. Static evidence is favorable, since `PyTestFuzzing`/`RTestFuzzing` occur in 14 test-source files and 0 main-source files, and the traced `test-classes` and `-tests.jar` mappings are both true. Confirm the CI testgen and Databricks test-publication jobs before treating the repair as fully exercised.
...
The first attempt hit a WSL-mounted filesystem I/O error. Moving generated
test output to native `/tmp` exposed a missing declared dataset; running the
repository's `getDatasets` task restored it and the final run passed.
Evidence is `spark40-final-packaging-complete.log` and
`spark40-final-packaging-evidence.json` in the session evidence directory.

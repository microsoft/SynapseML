...
- [x] Verified both Maven Central hostnames are handled by `MAVEN_CENTRAL_HOSTS` and used for probes and Coursier eviction in `tools/ci/sbt_retry.sh:80,132,245-246`; `tools/ci/tests/test_sbt_retry.py:464-465` asserts both resulting URLs.
- [x] Verified the canonical resolver exists at `build.sbt:314`, and the updated check at `tools/ci/tests/test_pipeline_yaml.py:117-120` is independent of whitespace and line wrapping.
...
- [x] Verified `path_has_symlink_component` at `tools/ci/sbt_retry.sh:176-190` checks every ancestor before `evict_cache_entry` reaches recursive removal, including symlinks within Ivy and Coursier coordinate paths.
- [x] Ran `test_intermediate_symlinks_cannot_redirect_eviction` independently under WSL: **1 passed**. The regression at `tools/ci/tests/test_sbt_retry.py:379-416` drives the wrapper end to end and verifies both external victims survive.
...
- [x] Reverified both Maven Central hostnames are shared by probing and Coursier eviction through `MAVEN_CENTRAL_HOSTS` at `tools/ci/sbt_retry.sh:80,132,265`, and the canonical fallback remains configured at `build.sbt:314`.
- [x] Ran `bash -n tools/ci/sbt_retry.sh`, Black checks on both changed Python test files, and `git diff a6fd536ad76eb1b60ac82f31a362ae624886c6ff --check`; all passed.

# Spark 4.1 CI repair review: attempt 2, round 5
...
- PR #2720 targets `spark4.1`; HEAD: `1c5f8921a7e6ebfd29dc49b24646877f1b3cc605`.
...
- [x] **Red/green requirement mapping**: CI #236207038 failure (`NoSuchMethodException` on `PyCodegenFixtures$TypedPythonEstimator.<init>()`) reproduced RED in `spark41-red-repro.log`. Green pass verified in `spark41-green-packaging.log` (27 tests/4 suites). Round 1 URL defect reproduced RED in `spark41-url-red.log` (2 failed tests) and GREEN in `spark41-url-green.log` (29 passed tests/4 suites, 0 failed/skipped). OpenAI stub `FileNotFoundError` reproduced RED in `spark41-openai-stub-red.log` and GREEN in `spark41-openai-stub-green.log`.
...
The pending actual-JAR generation check passed with native `/tmp` test output
and the declared dataset restored by `getDatasets`.
`spark41-final-packaging-evidence.json` records 81 Python and 67 R generated
test files, SelectColumns positive controls, and a verified production wheel
without fixture classes. `spark41-final-packaging-complete.log` records all
...
Black also passed again in `spark41-final-stub-check.log`.

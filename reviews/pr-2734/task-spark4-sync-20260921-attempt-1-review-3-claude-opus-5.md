...
- **Target**: spark4.1
- **Branch**: `sync/spark4.1-master-20260921`
...
- **Artifact**: `reviews/pr-2734/task-spark4-sync-20260921-attempt-1-review-3-claude-opus-5.md`
- **Issues Found**: 2 Low
...
The frozen staged sync candidate: 40 files, 3,728 additions, 202 deletions.
...
- [x] Confirmed the reviewed content by hash rather than by trust:
  `git write-tree` = `8a40c8312e62e3d7e650a851d968ddfc0b95a3d8`, and
  ...
  master prerequisite and to the Spark 4.0 candidate, so the robustness
  reasoning below transfers exactly.
...
- [x] **Scala 2.13 collection shape.** `values` matches on
  ...
  `build.sbt` / `environment.yml` for this branch's Scala and Spark pins; this
  is one of the port-specific adaptations the sync must preserve, and it is
  preserved.
...
- [x] **Persistence round-trip.** `_service_param_scalar_to_python` falls back
  ...
  covers both scalar and column bindings through `save`/`load`. The port's
  zero-argument `super()` adaptations in the prompt overrides are intact and do
...
- [x] Confirmed the port keeps its own adaptations in the reviewed robustness
  paths: Scala 2.13 collection handling, streaming and SAR encoder adaptations,
  the zero-argument `super()` fixes in the prompt overrides, and
  `condition: false` on the branch Fabric E2E job, so the preflight wiring
  reviewed above does not authorise deletions from this branch today.
...
Re-checked the two fixes only. Tree `d2bb363153`; blobs `e92bc35a94` (cleanup),
...
- **Green.** `spark41-cleanup-round3-green-v2.log`: scalastyle 0 errors at 800

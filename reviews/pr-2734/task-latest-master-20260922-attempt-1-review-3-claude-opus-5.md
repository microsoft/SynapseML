# Latest-master sync into spark4.1, round 3

## Review summary

- PR: microsoft/SynapseML#2734. Theme: edge cases and robustness.
- Model: Claude Opus 5.
- HEAD: `d6691ae29c20a87139e37ec6d5966f23b3b760dd`
- MERGE_HEAD: `0a7fdafaa33ff4785dadc8d7eebee68efde110fb`
- Merge base: `714d365e71f6d2db5b7072094a4a3ad22485eb57`
- Staged tree fingerprint: `7ba0eb1de595bc97ceddccf300362ec574822d42`
...
port and microsoft/SynapseML#2733 are textually identical apart from two blob index lines, so
...
This port carried Scala 2.13 path edits in the removed runner; deleting it is still correct, since no
tracked file invokes it and master owns the retirement.

## Fabric-disable boundary

Correct in the pipeline and tests. `FabricE2E` keeps `condition: false` with the Spark 4.1 runtime
...
`test_fabric_e2e_keeps_key_vault_authentication_while_disabled` and the Spark 4.1 runtime guard both
...
The Spark 4.1 guards remain intact alongside the adopted master changes: `test_spark41_ci_uses_supported_runtime`
(`SPARK_VERSION=4.1.1`), `test_spark41_isolation_forest_excludes_spark_runtime_dependencies`, and
`test_github_pr_validation_supports_spark41`.
...
disabled and its decision variable is not referenced. A reader on spark4.1 could conclude that an
...
not re-run; no Azure or live-service result is claimed. The reported one-off Git replay fixture index
mismatch under WSL and its clean retry were likewise not reproduced here.

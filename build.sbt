...
// A tag checkout alone is not permission to publish immutable release coordinates.
ThisBuild / version ~= { dynverVersion =>
  ReleaseVersion.resolve(dynverVersion, sys.env)
}
...
publishPypi := {
  ...
  runCmd(
    activateCondaEnv ++
      Seq("twine", "upload", "--non-interactive",
        join(rootGenDir.value, "package", "python", fn).toString),
    envVars = Map("TWINE_USERNAME" -> "__token__", "TWINE_PASSWORD" -> Secrets.pypiApiToken)
  )
}

...
object FabricTestConstants {
  ...
  private def createAndConfigureWorkspace(
    ...
    val createResponse = client.postRequest(
      "https://api.fabric.microsoft.com/v1/workspaces", createBody)
    val workspaceId = createResponse.asJsObject.fields("id").convertTo[String]
    println(s"Created workspace '$workspaceName' -> $workspaceId")

    // Step 2: Assign capacity — discover from the caller's available capacities
    ...
    val activeCapacity = capacityList.find { c =>
      c.fields.get("state").exists(_.convertTo[String] == "Active")
    }
    activeCapacity match {
      case Some(cap) =>
        ...
        val capacityBody = JsObject("capacityId" -> JsString(capacityId)).compactPrint
        // assignToCapacity returns 202 with empty body — use raw HTTP, not sendAndParseJson
        import org.apache.http.client.methods.HttpPost
        import org.apache.http.entity.StringEntity
        import com.microsoft.azure.synapse.ml.io.http.RESTHelpers.safeSend
        val assignReq = new HttpPost(
          s"https://api.fabric.microsoft.com/v1/workspaces/$workspaceId/assignToCapacity")
        client.setRequestContentTypeAndAuthorization(assignReq)
        assignReq.setEntity(new StringEntity(capacityBody))
        val assignResp = safeSend(assignReq, expectedCodes = Set(200, 202))
        assignResp.close()
        println(s"Assigned capacity '$capacityName' ($capacityId) to workspace $workspaceId")
      case None =>
        println("WARNING: No active capacity found. Workspace may not support Spark operations.")
    }

    // Wait for capacity assignment to propagate
    Thread.sleep(10000) //scalastyle:ignore

    // Step 3: Configure Spark runtime
    val sparkBody = JsObject(
      "environment" -> JsObject(
        "runtimeVersion" -> JsString(fabricRuntimeVersion)
      )
    ).compactPrint
    client.patchRequest(
      s"https://api.fabric.microsoft.com/v1/workspaces/$workspaceId/spark/settings",
      sparkBody, "*")

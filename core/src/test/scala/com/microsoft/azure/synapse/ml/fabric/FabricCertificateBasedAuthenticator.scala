...
class FabricCertificateBasedAuthenticator extends AutoCloseable {
  ...
  override def close(): Unit = {
    try {
      if (httpClientWithRedirect != null) httpClientWithRedirect.close()
    } catch {
      case _: Exception =>
    }
    try {
      if (httpClientWithoutRedirect != null) httpClientWithoutRedirect.close()
    } catch {
      case _: Exception =>
    }
  }
  ...
  def getTokenWithCertificateSilently(
    ...
    require(pfxFilePath.isDefined || pfxBase64Content.isDefined,
      "Either pfxFilePath or pfxBase64Content must be provided")
    ...
    try {
      ...
      val certAuthParameters = getCertAuthParameters(
        config.authority, keyStore, alias, password, certAuthUrl, authConfigMap)
      ...
      tokenResponse.fields("access_token").convertTo[String]
    } catch {
      case e: Exception =>
        throw new RuntimeException(
          s"Failed to obtain access token using certificate authentication: ${e.getMessage}", e)
    }
  }
  ...
  def refreshTokenPair(
    ...
    val newRefreshToken = tokenResponse.fields("refresh_token").convertTo[String]
      ...
      case Failure(_) =>
        if (passwordChars.nonEmpty) {
          Try {
            ...
            case Success(_) =>
            case Failure(e) =>
              throw new IllegalArgumentException(s"Failed to load certificate: ${e.getMessage}", e)
          }
        } else {
      ...
      case Failure(_) =>
        if (passwordChars.nonEmpty) {
          Try {
            ...
            case Success(_) =>
            case Failure(e) =>
              throw new IllegalStateException(s"Failed to initialize key manager: ${e.getMessage}", e)
          }
        } else {
    ...
    try {
      ...
      if (loginResponse.contains("strMainMessage")) {
        throw new IllegalArgumentException(
          loginResponse.getOrElse("strServiceExceptionMessage", "Authentication failed"))
      }
      ...
      val kmsiParams = List(
        new BasicNameValuePair("ctx", loginResponse("sCtx")),
        new BasicNameValuePair("flowtoken", loginResponse("sFT")),
        new BasicNameValuePair("canary", loginResponse("canary")),
        new BasicNameValuePair("hpgrequestid", loginResponse("sessionId")),
        new BasicNameValuePair("LoginOptions", "3"),
      ...
      try {
        val location = kmsiResponse.getFirstHeader("Location").getValue
        val uri = new URI(location)
        val fragment = uri.getFragment
        if (fragment != null) {
          val code = fragment.split("&")(0).split("=")(1)
  ...
  private def postMSOAuth2Token(
    ...
    tokenRequest.setHeader("Origin", redirectUri)
  ...
  private def parseAuthResponse(authResponse: String): Map[String, String] = {
    ...
    if (startIndex < startDelimiter.length) return authResponseMap.toMap

    val endIndex = authResponse.indexOf(endDelimiter, startIndex)
    if (endIndex == -1) return authResponseMap.toMap

    val jsonContent = authResponse.substring(startIndex, endIndex)

    Try {
      val json = jsonContent.parseJson.asJsObject
      json.fields.foreach { case (key, value) =>
        value match {
          case JsString(str) => authResponseMap(key) = str
          case JsNumber(num) => authResponseMap(key) = num.toString
          case JsBoolean(bool) => authResponseMap(key) = bool.toString
          case JsNull => authResponseMap(key) = ""
          case _ =>
        }
      }
    } match {
  ...
  private def parseHiddenInputs(htmlContent: String): Map[String, String] = {
    ...
    Try {
      ...
      for (i <- 0 until inputs.getLength) {
        val input = inputs.item(i)
        val attributes = input.getAttributes

        val typeAttr = Option(attributes.getNamedItem("type")).map(_.getNodeValue)
        ...
        (typeAttr, nameAttr, valueAttr) match {

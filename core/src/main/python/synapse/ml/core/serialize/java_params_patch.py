...
def _resolve_python_target(py_name):
    ...
    vs class mismatches).
    """
    # Security gate: restrict to trusted module prefixes (mirrors secure_import_class)
    from synapse.ml.core.serialize._safe_import import _ALLOWED_MODULE_PREFIXES

    if not py_name.startswith(_ALLOWED_MODULE_PREFIXES):
        raise ImportError(
            f"Refusing to load class '{py_name}': "

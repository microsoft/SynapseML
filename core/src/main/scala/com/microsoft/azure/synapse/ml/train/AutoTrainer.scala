...
import com.microsoft.azure.synapse.ml.core.contracts.{HasExcludedFeatures, HasFeaturesCol, HasLabelCol}
...
trait AutoTrainer[TrainedModel <: Model[TrainedModel]] extends Estimator[TrainedModel]
  with HasLabelCol with HasExcludedFeatures with ComplexParamsWritable with HasFeaturesCol with Wrappable {
  ...
  setDefault(numFeatures -> 0, excludedFeatures -> Array.empty[String])
}

...
object SynapsePlatformUtils {
  ...
  def currentPlatform: String = {
    sys.env.getOrElse("AZURE_SERVICE", None).toString match {
      case "Microsoft.ProjectArcadia" => getClusterType match {
  ...
  def insideSynapse: Boolean = currentPlatform != PlatformUnknown

  // This should only be used for PlatformSynapseInternal
  private[ml] def getAccessTokenInternal(resource: String = "pbi"): String = {
  ...
  // This should only be used for PlatformSynapseInternal
  private[ml] def getTokenExpiryTime(token: String): java.util.Date = {
    val clusterType = getClusterType
    clusterType match {

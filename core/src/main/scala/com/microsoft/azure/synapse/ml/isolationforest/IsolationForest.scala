...
import com.linkedin.relevance.isolationforest.{IsolationForest => IsolationForestSource,
...
class IsolationForest(override val uid: String, val that: IsolationForestSource)
  extends Estimator[IsolationForestModel]
    with IsolationForestParamsBase with DefaultParamsWritable with Wrappable with SynapseMLLogging {
...
class IsolationForestModel(override val uid: String)
  extends Model[IsolationForestModel]
    with IsolationForestParamsBase with ComplexParamsWritable with Wrappable with SynapseMLLogging {
  logClass(FeatureNames.IsolationForest)

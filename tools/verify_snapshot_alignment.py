...
corresponds to a locally available synapseml_2.13 aggregate and that
...
or CI for Spark 4 / Scala 2.13, to avoid cases where Python requests a
snapshot version that has not been fully published, which would cause
Spark startup to fail with unresolved dependency errors.
...
def _find_aggregate_pom(version: str) -> Path:
    m2_base = (
    ...
    agg_dir = m2_base / "synapseml_2.13" / version
    pom_path = agg_dir / f"synapseml_2.13-{version}.pom"
...
def main(argv: List[str]) -> int:
    ...
    pom_path = _find_aggregate_pom(version)
    ...
    print(
        f"OK: synapseml_2.13 aggregate and all referenced modules are present "

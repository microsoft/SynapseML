...
def test_spark41_ci_uses_supported_runtime():
    data = yaml.safe_load(_pipeline_text())
    jobs = {j.get("job"): j for j in _jobs(data["jobs"])}
    ...
    prepare_script = prepare["bash"]
    assert "SPARK_VERSION=4.1.1" in prepare_script
    assert "tar xzf spark-${SPARK_VERSION}-bin-hadoop${HADOOP_VERSION}.tgz" in (
        prepare_script
    )

    test_step = next(
        step for step in r_steps if step.get("displayName") == "Test R Code"
    )
    assert 'export SPARK_HOME="$(find $(Build.SourcesDirectory)/..' in (
        test_step["inputs"]["inlineScript"]
    )
...
def test_spark41_isolation_forest_excludes_spark_runtime_dependencies():
    build = BUILD_SBT.read_text()
    dependency_start = build.index('"com.linkedin.isolation-forest"')
    dependency_end = build.index(").map", dependency_start)
    dependency = build[dependency_start:dependency_end]

    for module in ("spark-avro", "spark-core", "spark-mllib", "spark-sql"):
        exclusion = f'exclude("org.apache.spark", s"{module}_$scalaMajorVersion")'
        assert exclusion in dependency


def test_github_pr_validation_supports_spark41():
    workflow = PR_VALIDATION.read_text()
    assert 'branches: [ "master", "spark4.1" ]' in workflow

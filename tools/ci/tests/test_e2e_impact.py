...
def test_non_pr_runs_do_not_even_consult_git(monkeypatch, tmp_path, reason):
    def unexpected_detection(*args):
        raise AssertionError("Non-PR builds must not inspect Git changes")

    monkeypatch.setattr("tools.ci.e2e_impact.changed_paths", unexpected_detection)
    assert (
        select_suites(
...
def test_forced_or_unknown_full_test_option_runs_everything(make_pr, override):
    repo, env = make_pr({PYTHON_TEST: "changed\n"})
    assert select_suites(repo, env) == frozenset()
    env["SYNAPSEML_FULL_TESTS"] = override
    assert select_suites(repo, env) == ALL_SUITES

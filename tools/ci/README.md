...
Docker builds read their Python version from `environment.yml` through this
helper. It accepts one numeric `major.minor` or `major.minor.patch` dependency
and emits it unchanged, preserving a port branch's minor-series selection.
Missing, multiple, or malformed version specifications fail the build.

...
object ONNXHubJsonProtocol extends DefaultJsonProtocol {
  ...
  implicit val MetaFormat: RootJsonFormat[ONNXMetadata] = jsonFormat(ONNXMetadata.apply,
    "model_sha",
    "model_bytes",
    "tags",
    "io_ports",
  ...
  implicit val InfoFormat: RootJsonFormat[ONNXModelInfo] = jsonFormat(ONNXModelInfo.apply,
    "model",
    "model_path",
    "onnx_version",
    "opset_version",
...
class ONNXHub(val modelCacheDir: Path) extends Logging {
  ...
  private def getBaseUrl(repo: String, lfs: Boolean = false): URL = {
    ...
    if (lfs) {
      new URL(s"https://media.githubusercontent.com/media/$repoOwner/$repoName/$repoRef/")
    } else {
      new URL(s"https://raw.githubusercontent.com/$repoOwner/$repoName/$repoRef/")
    }
  }
  ...
  def getModelInfo(model: String, repo: String = ONNXHub.DefaultRepo, opset: Option[Int] = None): ONNXModelInfo = {
    ...
    val selectedModels = opset.map(ov =>
      matchingModels.filter(_.opsetVersion == ov)
    ...
    selectedModel.metadata.modelSha.foreach { trueSha =>
      ...
      assert(downloadedSha == trueSha,
        s"Cached model has SHA256 $downloadedSha but checksum should be $trueSha, " +
          s"the model download might have failed. use forceReload to re-download model."
      )
    }
    modelBytes
  }

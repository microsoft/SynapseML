...
/** Abstract representation of a repository for future expansion
  ...
  */
private[ml] abstract class Repository[S <: Schema] {
...
private[ml] class HDFSRepo[S <: Schema](val uri: URI, val hconf: HadoopConf)
...
/** Class to represent repository of models that will eventually be hosted outside
  * the repo.
  */
private[ml] class DefaultModelRepo(val baseURL: URL) extends Repository[ModelSchema] {
...
private[ml] abstract class Client {
  var quiet = false
...
private[ml] object ModelDownloader {
  private[ml] val DefaultURL = new URL("https://mmlspark.azureedge.net/datasets/CNTKModels/")
}

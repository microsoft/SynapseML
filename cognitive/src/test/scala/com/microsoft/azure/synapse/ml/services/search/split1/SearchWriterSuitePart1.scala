...
package com.microsoft.azure.synapse.ml.services.search.split1
...
import com.microsoft.azure.synapse.ml.services.openai.OpenAIAPIKey
...
class SearchWriterSuiteUtilities extends TestBase with AzureSearchKey
  with IndexJsonGetter with IndexParser with CognitiveKey with OpenAIAPIKey {
  ...
  private[ml] def createTestDataWithVector(numDocs: Int): DataFrame = {
    (0 until numDocs)
      .map(i => ("upload", s"$i", s"file$i", Array(0.001, 0.002, 0.003).map(_ * i)))
      .toDF("searchAction", "id", "fileName", "vectorCol")
  }

  private[ml] def createSimpleIndexJson(indexName: String): String = {
    s"""
       |{
       |    "name": "$indexName",
       |    "fields": [
  ...
  private[ml] def createSimpleIndexJsonWithVector(indexName: String): String = {
    s"""
       |{
       |    "name": "$indexName",
       |    "fields": [
  ...
  private[ml] val createdIndexes = new mutable.HashMap[String, mutable.Set[String]]

  private[ml] val currentTestData = new ThreadLocal[TestData]
  ...
  private[ml] def generateIndexName(testName: Option[String] = None): String = {

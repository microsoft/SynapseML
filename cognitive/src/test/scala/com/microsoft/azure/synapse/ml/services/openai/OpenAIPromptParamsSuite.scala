...
class OpenAIPromptParamsSuite extends TestBase {
  ...
  private def generatedPythonClass: String =
    new OpenAIPrompt().generatedPythonClass
  ...
  test("Generated Python should contain validated setters and setParams implementation") {
    ...
    assert(generatedClass.contains("return super().clear(param)"))
    assert(generatedClass.contains("result = super().copy(extra)"))
    assert(!generatedClass.contains("super(OpenAIPrompt, self)"))

...
class AzMapsPointInPolygonSuite extends TransformerFuzzing[CheckPointInPolygon] with AzureMapsKey
  ...
  test("Point in Polygon: Basic Usage") {
    println(s"Using ${udidToUse} as the user-data identifier for these tests")
    pointInPolygonCheck.setUserDataIdentifier(udidToUse)
  ...
  override def afterAll(): Unit = {
    ...
    // Cleanup
    tryWithRetries() { () =>
      lazy val deleteRequest = new HttpDelete(new URI("https://us.atlas.microsoft.com/mapData/"
        + udidToUse + "?" + queryParams))
      deleteRequest.setHeader("User-Agent",
        s"synapseml/${BuildInfo.version}${HeaderValues.PlatformInfo}")
      lazy val longRunningResultResponse = client.execute(deleteRequest)
      lazy val resultStatusCode = longRunningResultResponse.getStatusLine.getStatusCode
  ...
  override def beforeAll(): Unit = {
    ...
    lazy val queryParams =  URLEncodingUtils.format(Map("api-version" -> "1.0",
      "subscription-key" -> azureMapsKey))
    lazy val createRequest = new HttpPost(new URI("https://us.atlas.microsoft.com/mapData/upload?" + queryParams +
      "&dataFormat=geojson"))
    createRequest.setHeader("Content-Type", "application/json")
    createRequest.setHeader("User-Agent", s"synapseml/${BuildInfo.version}${HeaderValues.PlatformInfo}")
    createRequest.setEntity(new StringEntity(testPolygon))
    lazy val response = client.execute(createRequest)
    assert(response.getStatusLine.getStatusCode === 202)
    lazy val locationUrl =  response.getFirstHeader("location").getValue
    assert(locationUrl != null )
    response.close()

    tryWithRetries(Array(3000, 5000, 10000, 20000, 30000)) { () =>
      lazy val getLongRunningResult = new HttpGet(new URI(locationUrl + "&" + queryParams))
      getLongRunningResult.setHeader("User-Agent",
        s"synapseml/${BuildInfo.version}${HeaderValues.PlatformInfo}")
      lazy val longRunningResultResponse = client.execute(getLongRunningResult)
      lazy val statusCode = longRunningResultResponse.getStatusLine.getStatusCode
      ...
      udidToUse = resourceLocation.split(Array('/','?', '&'))(5)
    }
  }

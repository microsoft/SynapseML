...
class TextSentiment(override val uid: String)
  ...
  logClass(FeatureNames.AiServices.Text)
  ...
  override def postprocessResponse(responseOpt: Row): Option[Seq[Row]] = {
    Option(responseOpt).map { response =>
      ...
      val flattenedTasks = Seq(
        flattenTask(Option(tasks.getAs[scala.collection.Seq[Row]]("entityRecognitionTasks")).map(_.toSeq).orNull),
        flattenTask(Option(tasks.getAs[scala.collection.Seq[Row]]("entityLinkingTasks")).map(_.toSeq).orNull),
        flattenTask(Option(tasks.getAs[scala.collection.Seq[Row]]("entityRecognitionPiiTasks")).map(_.toSeq).orNull),
        flattenTask(Option(tasks.getAs[scala.collection.Seq[Row]]("keyPhraseExtractionTasks")).map(_.toSeq).orNull),
        flattenTask(Option(tasks.getAs[scala.collection.Seq[Row]]("sentimentAnalysisTasks")).map(_.toSeq).orNull)

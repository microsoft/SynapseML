...
private object CNTKModelUtils extends java.io.Serializable {
  ...
  def applyCNTKFunction(model: SerializableFunction,
  ...
  /** This defines and instantiates an iterator, hasNext and next are the abstract methods that
    ...
    * to be evaluated.
    */
  def minibatchIterator(model: SerializableFunction,
                        inputVar: Variable,
  ...
  def applyModel(inputIndex: Int,
                 broadcastedModel: Broadcast[SerializableFunction],
                 minibatchSize: Int,
                 inputNode: Int,
    ...
    val outputNode: Option[SerializableFunction] = (outputNodeName, outputNodeIndex) match {
      case (Some(name), None) =>
...
class CNTKModel(override val uid: String) extends Model[CNTKModel] with ComplexParamsWritable
  ...
  private var broadcastedModelOption: Option[Broadcast[SerializableFunction]] = None

  /** @group setParam */
  def setModel(value: SerializableFunction): this.type = {
  ...
  /** @group getParam */
  def getModel: SerializableFunction = $(model)
  ...
  def setModelLocation(spark: SparkSession, path: String): this.type = {
    ...
    setModel(SerializableFunction.loadModelFromBytes(modelBytes))
  }

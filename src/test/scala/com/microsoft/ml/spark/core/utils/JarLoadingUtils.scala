...
object JarLoadingUtils {
  ...
  def instantiateServices[T: ClassTag](instantiate: Class[_] => Any): List[T] = {
    AllClasses
  ...
  def instantiateServices[T: ClassTag]: List[T] = instantiateServices[T] {
    clazz: Class[_] => clazz.getConstructor().newInstance()
  }

  def instantiateObjects[T: ClassTag]: List[T] = instantiateServices[T] { clazz: Class[_] => {

...
class TrainValidRecommendSplitRec
  ...
  test("testSplitWData") {
    ...
    val conf = new SparkConf()
      ...
      .set("spark.logConf", "true")
      .set("spark.sql.warehouse.dir", localWarehousePath)
      .set("spark.executor.extraJavaOptions", "-XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+UseG1GC -XX:InitiatingHeapOccupancyPercent=35 -XX:ConcGCThread=20")
    val sess = SparkSession.builder()
      .config(conf)
      .getOrCreate()
    sess.sparkContext.setLogLevel("WARN")

    //Implicit Map(map -> 0.020415022341847663, mapk -> 0.013762790449597308, maxDiversity -> 0.6402010050251257, diversityAtK -> 0.16532663316582916, recallAtK -> 0.013762790449597311, ndcgAt -> 0.04521755862022604)
    //Map(map -> 0.025498697400195208, mapk -> 0.014721797157235497, maxDiversity -> 0.9623115577889447, diversityAtK -> 0.48743718592964824, recallAtK -> 0.014721797157235495, ndcgAt -> 0.04688129124679781)Alert ProvidedAL - Suite TrainValidRecommendSplitRec took 2415.748s
    //40min
    val k = 10

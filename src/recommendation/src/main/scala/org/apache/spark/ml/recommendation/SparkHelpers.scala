...
trait RecommendationModelParams extends Params with ALSModelParams with HasPredictionCol {
  ...
  def recommendForAllUsers(userDataFrame: DataFrame, itemDataFrame: DataFrame, k: Int): DataFrame = {
    import userDataFrame.sparkSession.implicits._
    val itemRDD = if (true) {
      itemDataFrame.rdd.flatMap(row =>
      ...
      itemDataFrame.sparkSession.read.parquet("./items-parquet")
        .rdd.flatMap(row => row.getAs[mutable.WrappedArray[Float]](1).zipWithIndex.map(r =>
    ...
    val recs = sc.createDataFrame(userToItemMatrix)
      .toDF("id", "ratings").withColumn("recommendations", zipVector($"ratings")).select("id", "recommendations")

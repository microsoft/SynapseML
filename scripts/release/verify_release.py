...
from release_config import load_profile, package_name
...
def maven_artifact_filename(filename, module, version, allow_unversioned=False):
    suffix_pattern = (
        r"(?:\.pom|(?:-(?:sources|javadoc|tests|test-sources|test-javadoc|"
        r"tests-sources|tests-javadoc))?\.jar)"
        r"(?:\.asc)?(?:\.(?:md5|sha1|sha256|sha512))?"
    )
    prefix = f"{module}-{version}"
    if filename.startswith(prefix) and re.fullmatch(
        suffix_pattern, filename[len(prefix) :]
    ):
        return filename
    if (
        allow_unversioned
        and filename.startswith(module)
        and re.fullmatch(suffix_pattern, filename[len(module) :])
    ):
        return prefix + filename[len(module) :]
...
def validate_public_maven_inventory(artifacts, version, scala, pypi_version=None):
    ...
    for artifact in artifacts:
        ...
        if path != wheel:
            ...
            maven_artifact_filename(parts[1], parts[0], version)
        observed.add(path)
...
class Checker:
    def __init__(self, token: Optional[str], gh_token: Optional[str], skip: List[str]):
        ...
        profile = load_profile() if needs_ado or "internal" not in self.skip else None
        self.private_profile = profile
        self.internal_repository = (
            profile["internal_repository"]["id"] if profile else None
        )
        if needs_ado:
        ...
        self.pip_feed = profile["pip_feed"]["name"] if profile else None
        self.upack_feed = profile["upack_feed"]["name"] if profile else None
    ...
    def internal_maven(self, scala: str, version: str) -> str:
        if "internal" in self.skip:
            return SKIPPED
        return self._maven(
            package_name(self.private_profile, "internal", "maven"), scala, version
        )
    ...
    def ado_tag(self, tag: str) -> Tuple[str, Optional[str]]:
        ...
        url = (
            ...
            f"{self.internal_repository}/refs?filter=tags/{tag}&peelTags=true&api-version=7.1"
        )
        data = _json_get(url, self._ado_headers)
        if data is None:
            raise RuntimeError(
                f"Configured private repository refs endpoint not found: {url}"
            )
        refs = data.get("value")
        if not isinstance(refs, list):
            raise RuntimeError(
                "Configured private repository refs response has no value list"
            )
        ...
        if not isinstance(commit, str) or not commit:
            raise RuntimeError(
                f"Configured private repository tag {tag} has no object ID"
            )
        return OK, commit
...
def _check_plan(plan, token, gh_token, skip, strict, checker=None):
    private_packages = (
        {
            family: package_name(plan.private_profile, "internal", family)
    ...
    c.private_profile = plan.private_profile
    c.internal_repository = (
        plan.private_profile["internal_repository"]["id"]
        if plan.private_profile
        else None
    )
    rows: List[dict] = []
    ...
    for tp in plan.targets:
        ...
        if include_internal and "maven" in plan.families:
            add(
                "maven",
                tp.key,
                f"{private_packages['maven']}_{tp.scala}",
...
def validate_inventory(plan, report, max_age_seconds=3600):
    ...
    elif "internal_patch" in report:
        raise ValueError("public evidence must omit private fields")
    if (
        type(report.get("schema_version")) is not int
        or report.get("inventory_complete") is not True
    ):
        raise ValueError("release evidence is not a complete versioned inventory")
...
def validate_public_evidence(report):
    ...
    try:
        ...
        if not isinstance(payload, str) or len(payload) > 65536:
            raise ValueError("invalid public evidence plan payload")
        plan = load_plan(
            parse_plan_json(base64.b64decode(payload, validate=True)),
            require_bound=True,
        )
    except (KeyError, TypeError, binascii.Error, UnicodeError) as error:
...
def main(argv=None) -> int:
    ...
    source.add_argument("--plan", help="Use a bound, reviewed versioned JSON plan")
    ...
    p.add_argument(
        ...
        help=(
            ...
            "pip (all configured private-feed wheels), internal "
    ...
    if args.github_evidence:
        ...
        print(f"SynapseML verification  OSS v{report['version']}  " f"scope={scope}")
        if plan:
            print(

...
def _feed_identity(remote, requested, profile):
    try:
    ...
    if (
        ...
        or data["project"]["id"].lower() != profile["project_id"]
...
def _destinations(plan, remote):
    ...
    if not selected:
        return {}
    profile = plan.private_profile
    checked = {"pip", "upack"} if plan.mode == "rehearsal" else selected
    resolved = {
        family: _feed_identity(remote, getattr(plan, family + "_feed"), profile)
        for family in sorted(checked)
    }
    if plan.mode == "production" and any(
        feed["id"] != profile[family + "_feed"]["id"]
        for family, feed in resolved.items()
    ):
        raise ReleaseError(
    ...
    if plan.mode == "rehearsal":
        production = [
            _feed_identity(remote, profile[family + "_feed"]["name"], profile)
            for family in ("pip", "upack")
        ]
        ids = {feed["id"] for feed in production} | {
            profile[family + "_feed"]["id"] for family in ("pip", "upack")
        }
...
def _operation(plan, action, families):
    matrix.require_execution_plan(plan)
    ...
    if action["pipeline_id"] != expected_pipeline:
        raise ReleaseError("Operation changed the approved pipeline destination")
    if action["pipeline_id"] == matrix.OSS_MAVEN_PIPELINE_ID:
        matrix.require_public_plan(plan)
...
def _artifact_rows(report, repository, target, family, plan):
    selected = next(item for item in plan["targets"] if item["key"] == target)
    version = selected[f"{repository}_{family}_version"]
    if family == "maven" and repository == "oss":
        expected = {
            (kind, f"{module}_{selected['scala']}", version)
            for kind in ("maven", "maven-central")
        ...
        if family == "maven":
            name += "_" + selected["scala"]
        expected = {(family, name, version)}
    rows = []
    ...
    for report in [state["inventory"]] + state["dependency_inventory"]:
        rows = [
            ...
            for row in _artifact_rows(
                report, repository, target, "maven", state["plan"]
            )
            if row["kind"] == "maven"
        ]
        if rows:
    ...
    if action["kind"] == "maven":
        if action["repository"] == "oss":
            if (
                ...
                or expected_repo["name"].casefold() != "microsoft/synapseml"
            ):
                raise ReleaseError(
            ...
            or expected_repo["name"]
            != plan.private_profile["internal_repository"]["name"]
            or expected_repo["id"] != plan.private_profile["internal_repository"]["id"]
        ):
            raise ReleaseError(
...
def _public_jobs(jobs, from_timeline=False):
    ...
    for job in checked:
        job_id = job["id"].lower()
        if (
            not GUID_RE.fullmatch(job_id)
            or uuid.UUID(job_id).int == 0
            or job_id in seen
            or (not from_timeline and job_id != job["id"])
        ):
            raise ReleaseError("Public producer jobs require unique nonzero GUID IDs")
        seen.add(job_id)
        job["id"] = job_id
        if from_timeline:
            # Release is the publication job in pipeline.yaml. Other labels are
            # not part of the public evidence contract, but all outcomes still count.
            job["name"] = (
                "Release" if job["name"] == "Release" else "Maven pipeline job"
            )
...
def _validate_manifests(
    ...
    for document in documents:
        ...
        if action["kind"] == "publisher":
            package = matrix.package_name(
                plan.private_profile, action["repository"], family
            )
        ...
        if action["repository"] == "oss" and family == "maven":
            try:
                verify.validate_public_maven_inventory(
                    artifacts,
...
def _report(state, apply):
    ...
    return {
        ...
        "human_gates": (
            "Pipeline approvals remain manual. This command never creates tags, GitHub releases or deployment configuration changes."
        ),
    }
...
def _evidence_build(build, operation):
    ...
    for key, expected in operation["parameters"].items():
        if key not in parameters or not _parameter_equal(parameters[key], expected):
            raise ReleaseError(
                "Producer evidence parameters differ from the approved operation"
            )
        normalized_parameters[key] = (
            expected if type(expected) is bool else parameters[key]
        )
    return {
        "id": build["id"],
        ...
        "templateParameters": normalized_parameters,
...
def _validate_evidence_destinations(plan, destinations):
    ...
    for family, feed in destinations.items():
        ...
        elif feed["id"].lower() in {
            plan.private_profile[name + "_feed"]["id"] for name in ("pip", "upack")
        }:

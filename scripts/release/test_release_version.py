...
TaskKey[Unit]("releaseVersionChecks") := {
  ...
  assert(ReleaseVersion.mayOverwrite("icons", "badge.svg"))
  for (container <- Seq("new-release-container", "", "Maven", "maven ", "DOCS");
       destination <- Seq("release/1.1.4", "release/1.1.4-SNAPSHOT")) {
    val failure = scala.util.Try(ReleaseVersion.mayOverwrite(container, destination)).failed.get
    assert(failure.isInstanceOf[IllegalArgumentException])
    assert(failure.getMessage.contains("Unsupported publication container"))
  }

...
v1.1.3 / v1.1.1 releases (github tags + synthetic-private-upack + synthetic-private-pip),
...
def test_default_plan_is_public_oss_without_internal_or_private_feed_work():
...
def test_master_carries_three_oss_tags():
    "The anchor includes version, Spark and Python tags."
...
def test_text_plan_emits_every_selected_maven_build():
    ...
    assert text.count("pipeline=17563") == 2
    assert text.count("pipeline=900001") == 2
...
def test_internal_superpatch_flows_everywhere():
    "A private patch updates only private artifact versions."
...
def test_internal_only_hotfix_never_republishes_oss():
    ...
    assert text.count("pipeline=900001") == 1

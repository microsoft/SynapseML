...
import pytest
from test_release_config import PUBLISH_PIPELINE_ID
from test_release_config import INTERNAL_MAVEN_PIPELINE_ID
from test_release_config import PIP_FEED
from test_release_config import UPACK_FEED
...
PROD_PIP_ID = "11111111-1111-1111-1111-111111111111"
PROD_UPACK_ID = "22222222-2222-2222-2222-222222222222"
...
class FakeRemote:
    ...
    def __init__(self, monkeypatch, missing=()):
        ...
        self.feeds = {
            PIP_FEED: {"id": PROD_PIP_ID, "name": PIP_FEED},
            UPACK_FEED: {"id": PROD_UPACK_ID, "name": UPACK_FEED},
    ...
    def definition(self, pipeline_id):
        repositories = {
            ...
            INTERNAL_MAVEN_PIPELINE_ID: {
                ...
                "type": "TfsGit",
                "url": (
                    "https://dev.azure.com/msdata/A365/" "_git/synthetic-private-source"
                ),
            },
            PUBLISH_PIPELINE_ID: {
    ...
    def absence(self, plan, actions, destinations):
        ...
        for item in actions:
            ...
            package = (
                "synapseml"
                if repository == "oss"
                else plan.private_profile["internal_packages"][family]
            )
            artifacts.append(
                {
                    "action_id": item["id"],
    ...
    def register(self, command, build_id):
        ...
        if pipeline_id != PUBLISH_PIPELINE_ID:
...
def cli(tmp_path, monkeypatch, capsys):
    ...
    def invoke(command="resume", plan=None, apply=False, extra=(), state=True):
        chosen = plan or release_plan(repositories=["oss"], families=["maven"])
...
def test_wait_advances_only_approved_dependencies(
    ...
    plan = release_plan(
        ...
        families=["maven"] if repository == "oss" else ["maven", "pip", "upack"],
    )
    ...
    expected_runs = 1 if repository == "oss" else 2
    assert len(cli.remote.queued) == expected_runs
    assert len(wait_clock.delays) == expected_runs
...
def test_status_wait_leaves_downstream_queueing_to_approved_resume(cli, wait_clock):
    ...
    assert cli(plan=plan, apply=True)[0] == 1
    wait_clock.on_sleep = lambda: cli.remote.succeed(101, plan, "internal", ["maven"])
    code, report, error = cli("status", plan=plan, extra=["--wait"])
    assert code == 1
    assert not report["complete"]
    assert len(cli.remote.queued) == 1
    assert "manual" in error.lower()
    assert action(report, "internal", "maven")["status"] == "complete"
    assert action(report, "internal", "upack")["status"] == "planned"
...
def test_only_missing_selected_oss_family_is_enabled(cli):
    cli.remote.missing = {("oss", "upack")}
    plan = release_plan(families=["pip", "upack"])
    code, _, error = cli(plan=plan, apply=True)
    assert code == 1, error
    assert len(cli.remote.queued) == 1
    command = cli.remote.queued[0]
    assert command[command.index("--id") + 1] == str(PUBLISH_PIPELINE_ID)
    parameters = command_values(command, "--parameters")
...
def test_maven_queue_has_exact_release_source_and_contract_parameters(cli):
    ...
    cli.remote.succeed(101, plan, "oss", ["maven"])
    assert cli(plan=plan, apply=True)[0] == 0
    plan = release_plan(families=["maven"], repositories=["internal"])
    code, _, error = cli(
        plan=plan,
        apply=True,
        extra=["--state", str(cli.state.with_name("internal-state.json"))],
    )
    assert code == 1, error
    assert len(cli.remote.queued) == 2
    command = cli.remote.queued[-1]
    assert command[command.index("--id") + 1] == "900001"
...
def produced_maven_receipt(plan, build_id, root, target_key="master"):
    ...
    for module in verify.PUBLIC_MAVEN_MODULES:
        ...
        for suffix in suffixes:
            path = directory / (artifact + "-" + target.oss_maven_version + suffix)
            if suffix == ".pom":
                path.write_text(
                    "<project><groupId>com.microsoft.azure</groupId>"
                    f"<artifactId>{artifact}</artifactId>"
                    f"<version>{target.oss_maven_version}</version></project>",
                    encoding="utf-8",
                )
            else:
...
def test_public_maven_receipt_requires_exact_complete_artifact_inventory(
    cli, tmp_path, corruption
):
    plan = release_plan(families=["maven"], repositories=["oss"])
    cli.remote.missing = {("oss", "maven")}
    assert cli(plan=plan, apply=True)[0] == 1
    ...
    if corruption.startswith("missing-"):
        receipt["artifacts"] = [
            ...
            if not (
                item["path"].startswith("synapseml-vw_")
                if corruption == "missing-module"
                else item["path"].endswith("-tests.jar")
            )
        ]
    elif corruption == "empty-file":
        receipt["artifacts"][0]["size"] = 0
    else:
        item = copy.deepcopy(receipt["artifacts"][0])
        item["path"] = item["path"].replace(
            "-1.1.4.pom",
            "-9.9.9.pom" if corruption == "wrong-version" else "-1.1.4-SNAPSHOT.pom",
        )
...
def test_public_maven_and_private_publisher_require_separate_approvals(cli):
    combined = release_plan(families=["maven", "upack"], repositories=["oss"])
    assert cli(plan=combined, apply=True)[0] == 2
    assert not cli.remote.queued
    assert not cli.remote.inventory_calls
    plan = release_plan(families=["maven"], repositories=["oss"])
    ...
    cli.remote.succeed(101, plan, "oss", ["maven"])
    assert cli(plan=plan, apply=True)[0] == 0
    private_plan = release_plan(families=["upack"], repositories=["oss"])
    assert (
        cli(
            plan=private_plan,
            apply=True,
            extra=["--state", str(cli.state.with_name("private-state.json"))],
        )[0]
        == 1
    )
    assert len(cli.remote.queued) == 2
    ...
    assert publisher["release_plan_id"] == private_plan.plan_id
    assert json.loads(
        base64.b64decode(publisher["release_plan_base64"])
    ) == matrix.plan_to_dict(private_plan)
...
def test_rehearsal_feed_guard_resolves_names_and_ids_fail_closed(cli, fault):
    ...
    cli.remote.missing = {("oss", "upack")}
    if fault == "alias":
        cli.remote.feeds["release-test-upack"]["name"] = UPACK_FEED
    elif fault == "id":
...
def test_subprocess_runner_never_invokes_cmd_shell_or_leaks_stderr(
    ...
    value = ops.CommandRunner().json(["az", "pipelines", "run", "--id", "900002"])
    assert value == {"id": 42}
...
def test_public_notes_export_fits_github_and_passes_the_real_guard(
    ...
    for run in report["producer_evidence"]["runs"]:
        ...
        assert all("internal_commit" not in target for target in sealed["targets"])
    payload = {
        "ref": "v1.1.4",
        "inputs": {

...
def test_legacy_combined_identity_remains_sealed_and_read_only():
    ...
    resign(data)
    approved_identity = data["plan_id"]
    assert approved_identity != plan.plan_id
    assert matrix.plan_to_dict(matrix.load_plan(data, require_bound=True)) == data
    legacy = matrix.load_plan(data, require_bound=True)
    assert legacy.plan_id == approved_identity
    with pytest.raises(ValueError, match="regenerate and reapprove"):
        matrix.require_execution_plan(legacy)
...
def test_rehearsal_needs_explicit_isolated_destinations():
    ...
    for feed in ("synthetic-private-pip", "A365/synthetic-private-pip"):

...
def _git(repo, *arguments, input_text=None):
    try:
        ...
        raise ValueError(
            f"git {arguments[0]} timed out; inspect remote state before retrying"
        ) from error
...
def _is_ancestor(repo, commit, head):
    try:
        result = subprocess.run(
            [
                "git",
        ...
        raise ValueError("primary integration ancestry check timed out") from error
    if result.returncode not in (0, 1):
        raise ValueError("cannot verify primary integration ancestry")
    return result.returncode == 0


def _associated_pull_requests(commit):
    projection = (
        "[.[] | {number, state, merged: (.merged_at != null), "
        "head_sha: .head.sha, head_ref: .head.ref, head_repo: .head.repo.full_name, "
        "base_ref: .base.ref, base_repo: .base.repo.full_name, merge_commit_sha}]"
    )
    try:
        result = subprocess.run(
            [
                "gh",
                "api",
                f"repos/microsoft/SynapseML/commits/{commit}/pulls?per_page=100",
                "--hostname",
                "github.com",
                "--header",
        ...
        raise ValueError("cannot query canonical pull-request integration") from error
    if result.returncode or len(result.stdout.encode("utf-8")) > 60000:
        raise ValueError("cannot read bounded canonical pull-request integration")
    try:
        records = parse_plan_json(result.stdout)
    except ValueError as error:
        raise ValueError(
            "invalid canonical pull-request integration response"
        ) from error
    if (
        ...
        raise ValueError("invalid canonical pull-request integration response")
    return records


def verify_primary_integration(repo, tag, commit):
    if not isinstance(tag, str) or not re.fullmatch(r"v[0-9]+\.[0-9]+\.[0-9]+", tag):
        raise ValueError("primary integration requires a vX.Y.Z tag")
    if not isinstance(commit, str) or not re.fullmatch(r"[0-9a-f]{40}", commit):
        raise ValueError("primary integration requires a full source commit")
    if _git(repo, "remote", "get-url", "origin") not in {
        "https://github.com/microsoft/SynapseML",
        "https://github.com/microsoft/SynapseML.git",
        "git@github.com:microsoft/SynapseML.git",
        "ssh://git@github.com/microsoft/SynapseML.git",
    }:
        raise ValueError("primary integration requires a canonical origin")
    if _git(repo, "rev-parse", "--is-shallow-repository") != "false":
        raise ValueError(
            "primary integration requires complete, non-shallow Git history"
        )
    ...
    master = _git(repo, "rev-parse", "refs/remotes/origin/master")
    if _is_ancestor(repo, commit, master):
        return "master ancestry"
    for record in _associated_pull_requests(commit):
        if (
            record.get("state") == "closed"
            and record.get("merged") is True
            and record.get("head_sha") == commit
            and record.get("head_ref") == f"release-candidate/{tag}-master"
            and record.get("head_repo") == "microsoft/SynapseML"
            and record.get("base_ref") == "master"
            and record.get("base_repo") == "microsoft/SynapseML"
            and type(record.get("number")) is int
            and record["number"] > 0
            and isinstance(record.get("merge_commit_sha"), str)
            and re.fullmatch(r"[0-9a-f]{40}", record["merge_commit_sha"])
            and _is_ancestor(repo, record["merge_commit_sha"], master)
        ):
            return f"merged PR #{record['number']}"
    raise ValueError(
        "cannot confirm primary released source is integrated into master; merge the unchanged "
        "canonical release-candidate PR before publishing notes"
    )
...
def main(argv=None):
    ...
    integration = commands.add_parser("verify-primary-integration")
    integration.add_argument("--repo", type=Path, default=Path("."))
    integration.add_argument("--tag", required=True)
    ...
    try:
        ...
        elif args.command == "verify-primary-integration":
            basis = verify_primary_integration(args.repo, args.tag, args.commit)
            print(
                json.dumps(
                    {"tag": args.tag, "commit": args.commit, "integration": basis}
                )
            )
